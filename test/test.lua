...
local DBIns, Table, fields, OrderBy = DBClass({
    new_table = true,
    path = "database.db",
    type = "sqlite3",
    DEBUG = true,
...
local desc = OrderBy.DESC
local asc = OrderBy.ASC
...
local User = Table({
    ...
    time_create = fields.DateTimeField({
        null = true
    })
})
...
local News = Table({
    ...
    create_user_id = fields.ForeignKey({
        to_table = User
    })
})
...
function TestORMSelect:test04()
    local usernames = {"first", "second", "third", "operator", "creator", "randomusername"}
    local passwords = {"secret_one", "scrt_tw", "hello", "world", "testpasswd", "new"}
...
function TestORMSelect:test08_order()
    local users = User.get:orderBy({desc('id'), asc('username')}):all()
...
function TestORMSelect:test10_group()
    local user = User.get:groupBy({'age'}):all()
...
function TestORMSelect:test11_having()
    local user1 = User.get:groupBy({'id', 'password'}):having({
        age__gt = 40,
        id__notin = {1, 2, 3, 4, 5}
    }):all()
...
function TestORMSelect:test12_join()
    ...
    assertEquals(user_group:foreign('user').id, 1)
...
function TestORMSelect:test13_join_with_all()
    ...
    assertEquals(users[1]:references('news'):count(), 3)
end

function TestORMSelect:test14_update_many()
    local users_query = User.get:where({
        age__gt = 40
    })
